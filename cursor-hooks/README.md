# keepmind Cursor Hooks Integration
...
```bash
# Clone and build
git clone https://github.com/ManuelStaggl/keepmind.git
cd keepmind && bun install && bun run build
...
The hooks bridge Cursor's hook system to keepmind's worker API, allowing:
...
1. **Install**: `keepmind cursor install` generates initial context
2. **Stop hook**: Updates context in `.cursor/rules/keepmind-context.mdc` after each session
...
- **MCP Tools**: Configure keepmind's MCP server for `search`, `timeline`, `get_observations` tools
...
keepmind cursor install user

# Or install for current project only
keepmind cursor install
```
...
|-------------|--------|---------|
| `beforeSubmitPrompt` | `session-init.sh` | Initialize keepmind session |
...
- Initializes a new session in keepmind using `conversation_id` as the session ID
- Extracts project name from workspace root
- Outputs `{"continue": true}` to allow prompt submission

### Context Hook (`context-inject.sh`)
- Ensures keepmind worker is running before session
- Outputs `{"continue": true}` to allow prompt submission
...
- Maps them to keepmind's observation format
...
- Requests summary generation from keepmind
- **Updates context file** in `.cursor/rules/keepmind-context.mdc` for next session

## Configuration

The hooks read configuration from `~/.keepmind/settings.json`:
...
2. Check worker logs:
   ```bash
   tail -f ~/.keepmind/logs/worker-$(date +%Y-%m-%d).log
   ```
...
- [keepmind on GitHub](https://github.com/ManuelStaggl/keepmind)
- [Cursor Hooks Reference](../docs/context/cursor-hooks-reference.md)
- [keepmind architecture notes](https://github.com/ManuelStaggl/keepmind)
