...
1. **Install**: `keepmind cursor install` creates initial context file
2. **Stop hook**: `session-summary.sh` updates context after each session ends
3. **Cursor**: Automatically includes `.cursor/rules/keepmind-context.mdc` in all chats
...
3. Fetches existing context from keepmind and writes to `.cursor/rules/keepmind-context.mdc`
...
cat > .cursor/rules/keepmind-context.mdc << EOF
---
alwaysApply: true
---
# Memory Context
...
Located at: `.cursor/rules/keepmind-context.mdc`
...
[Your context from keepmind appears here]
...
When you run `keepmind cursor install`, the project is registered in `~/.keepmind/cursor-projects.json`. This allows the worker to automatically update your context file whenever a new summary is generated - even if it happens from Claude Code or another IDE working on the same project.

To see registered projects:
```bash
cat ~/.keepmind/cursor-projects.json
```
...
| Initial setup | Automatic | `keepmind cursor install` creates initial context |
| MCP tool access | ✅ Full support | ✅ Full support |
| Web viewer | ✅ Available | ✅ Available |

## First Session Behavior

When you run `keepmind cursor install`:
...
Configure keepmind's MCP server in Cursor for search tools:
...
Ask the agent: "Check keepmind for any previous work on authentication"

## File Location

The context file is created at:
```
<workspace>/.cursor/rules/keepmind-context.mdc
```

This is version-controlled by default. Add to `.gitignore` if you don't want to commit it:
```
.cursor/rules/keepmind-context.mdc
```
