...
3. **Parity with Claude Code**: Matches keepmind's hook behavior where possible
...
**Purpose**: Initialize keepmind session when prompt is submitted
...
- ✅ Observation format matches keepmind structure
...
**How It Works**:
1. Fetches context from keepmind worker
2. Writes to `.cursor/rules/keepmind-context.mdc` with `alwaysApply: true`
...
6. Write context to `.cursor/rules/keepmind-context.mdc`
...
3. **Context Injection**:
   - ✅ Solved via auto-updated `.cursor/rules/keepmind-context.mdc`
