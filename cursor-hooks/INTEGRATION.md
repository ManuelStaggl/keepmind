# keepmind ↔ Cursor Integration Architecture

## Overview

This integration connects keepmind's persistent memory system to Cursor's hook system, enabling:
...
session-init.sh
    ...
    └─ Initialize session in keepmind
    ↓
context-inject.sh
...
save-observation.sh
    ...
    ├─ Map to keepmind observation format
...
| Cursor Field | keepmind Field | Notes |
|-------------|------------------|-------|
...
| Cursor Event | keepmind Tool Name | Input Format |
|-------------|---------------------|--------------|
...
### Worker Settings
Located in `~/.keepmind/settings.json`:
...
1. **Context Injection**: Cursor's `beforeSubmitPrompt` doesn't support prompt modification. Context must be retrieved via:
   - MCP tools (keepmind provides search tools)
