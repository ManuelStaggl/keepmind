...
The room is one shared file (default `~/.keepmind/STANDUP.md`): YAML front
...
> `~/.keepmind/STANDUP.md`; speak with `node "<skill-dir>/standup.mjs" post …`,
...
```
Defaults: agent = git branch, file = `~/.keepmind/STANDUP.md`. Every write is
atomically locked.
