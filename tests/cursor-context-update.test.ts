...
function readContextFile(workspacePath: string): string | null {
  const rulesFile = join(workspacePath, '.cursor', 'rules', 'keepmind-context.mdc');
...
describe('Cursor Context Update', () => {
  ...
  describe('writeContextFile', () => {
    ...
    it('creates keepmind-context.mdc file', () => {
      writeContextFile(workspacePath, 'test context');
    ...
    it('includes description in frontmatter', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain('description: "keepmind context from past sessions (auto-updated)"');
    });
...
- Added new feature`;
    ...
    it('includes footer with MCP tools mention', () => {
      ...
      const content = readContextFile(workspacePath);
      expect(content).toContain("Use keepmind's MCP search tools for more detailed queries");
    });

    it('uses atomic write (no temp file left behind)', () => {
      writeContextFile(workspacePath, 'test context');

      const tempFile = join(workspacePath, '.cursor', 'rules', 'keepmind-context.mdc.tmp');
