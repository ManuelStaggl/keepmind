...
describe('OpenCode installer config registration', () => {
  ...
  it('adds keepmind to an existing plugin array', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode'],
      mcp: { context7: { enabled: true } },
    });

    expect(config.plugin).toEqual(['context-mode', './plugins/keepmind.js']);
    expect(config.mcp).toEqual({ context7: { enabled: true } });
  });

  it('does not duplicate an existing keepmind plugin reference', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode', './plugins/keepmind.js'],
    });
  ...
  it('creates opencode.json when missing', () => {
    ...
    expect(config.plugin).toEqual(['./plugins/keepmind.js']);
  });
