...
describe('runtime-selector', () => {
  beforeEach(() => {
    mockSettings = {
      ...
      KEEPMIND_SERVER_URL: '',
      KEEPMIND_SERVER_API_KEY: '',
      KEEPMIND_SERVER_PROJECT_ID: '',
      KEEPMIND_SERVER_BETA_URL: '',
      KEEPMIND_SERVER_BETA_API_KEY: '',
      KEEPMIND_SERVER_BETA_PROJECT_ID: '',
    };
    warnLogs.length = 0;
  });
  ...
  it("selectRuntime returns 'server' when KEEPMIND_RUNTIME='server' (canonical)", () => {
    mockSettings.KEEPMIND_RUNTIME = 'server';
    expect(selectRuntime()).toBe('server');
  });

  it("selectRuntime returns 'server' when KEEPMIND_RUNTIME='server-beta' (legacy back-compat)", () => {
    mockSettings.KEEPMIND_RUNTIME = 'server-beta';
    expect(selectRuntime()).toBe('server');
  });

  it('selectRuntime returns worker for unknown values', () => {
    mockSettings.KEEPMIND_RUNTIME = 'something-else';
    expect(selectRuntime()).toBe('worker');
  });
  ...
  it('resolveRuntimeContext falls back to worker when api key is missing', () => {
    mockSettings.KEEPMIND_RUNTIME = 'server';
    mockSettings.KEEPMIND_SERVER_URL = 'http://localhost:1234';
    mockSettings.KEEPMIND_SERVER_PROJECT_ID = 'p1';
    const ctx = resolveRuntimeContext();
    expect(ctx.runtime).toBe('worker');
  ...
  it("resolveRuntimeContext returns 'server' context when canonical keys are configured", () => {
    mockSettings.KEEPMIND_RUNTIME = 'server';
    mockSettings.KEEPMIND_SERVER_URL = 'http://localhost:1234';
    mockSettings.KEEPMIND_SERVER_API_KEY = 'cmem_xyz';
    mockSettings.KEEPMIND_SERVER_PROJECT_ID = 'project-uuid';
    const ctx = resolveRuntimeContext();
  ...
  it("resolveRuntimeContext returns 'server' context when legacy KEEPMIND_RUNTIME='server-beta' + legacy *_BETA_* keys are configured", () => {
    // Simulates an existing installed settings.json from before the rename.
    mockSettings.KEEPMIND_RUNTIME = 'server-beta';
    mockSettings.KEEPMIND_SERVER_BETA_URL = 'http://legacy.example:9999';
    mockSettings.KEEPMIND_SERVER_BETA_API_KEY = 'legacy_key';
    mockSettings.KEEPMIND_SERVER_BETA_PROJECT_ID = 'legacy-project';
    const ctx = resolveRuntimeContext();
  ...
  it('buildServerContext prefers new keys when both are set', () => {
    mockSettings.KEEPMIND_SERVER_URL = 'http://new.example:1111';
    mockSettings.KEEPMIND_SERVER_API_KEY = 'new_key';
    mockSettings.KEEPMIND_SERVER_PROJECT_ID = 'new-project';
    mockSettings.KEEPMIND_SERVER_BETA_URL = 'http://old.example:9999';
    mockSettings.KEEPMIND_SERVER_BETA_API_KEY = 'old_key';
    mockSettings.KEEPMIND_SERVER_BETA_PROJECT_ID = 'old-project';
  ...
  it('buildServerContext falls back to legacy *_BETA_* keys when new keys are unset', () => {
    // No KEEPMIND_SERVER_* keys set, but legacy ones are.
    mockSettings.KEEPMIND_SERVER_BETA_URL = 'http://legacy.example:9999';
    mockSettings.KEEPMIND_SERVER_BETA_API_KEY = 'legacy_key';
    mockSettings.KEEPMIND_SERVER_BETA_PROJECT_ID = 'legacy-project';
    const ctx = buildServerContext();
  ...
  // #2564 — switching KEEPMIND_RUNTIME flips which runtime hooks dispatch to
  ...
  it('flips worker <-> server when the setting changes (no reinstall)', () => {
    // Start on worker.
    mockSettings.KEEPMIND_RUNTIME = 'worker';
    ...
    mockSettings.KEEPMIND_RUNTIME = 'server-beta';
    mockSettings.KEEPMIND_SERVER_BETA_URL = 'http://localhost:9999';
    mockSettings.KEEPMIND_SERVER_BETA_API_KEY = 'cmem_flip';
    mockSettings.KEEPMIND_SERVER_BETA_PROJECT_ID = 'proj-flip';
