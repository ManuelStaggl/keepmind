...
describe('installerError decision logic', () => {
  ...
  beforeEach(() => {
    ...
    prevDataDir = process.env.KEEPMIND_DATA_DIR;
    process.env.KEEPMIND_DATA_DIR = home;
  });

  afterEach(() => {
    if (prevDataDir === undefined) delete process.env.KEEPMIND_DATA_DIR;
    else process.env.KEEPMIND_DATA_DIR = prevDataDir;
...
describe('cross-IDE failure matrix (11 IDEs x 3 scenarios)', () => {
  ...
  beforeEach(() => {
    prevMatrixDataDir = process.env.KEEPMIND_DATA_DIR;
    process.env.KEEPMIND_DATA_DIR = mkdtempSync(join(tmpdir(), 'cm-matrix-'));
  });
  afterEach(() => {
    const dir = process.env.KEEPMIND_DATA_DIR;
    ...
    // call-time resolvers.
    if (prevMatrixDataDir === undefined) delete process.env.KEEPMIND_DATA_DIR;
