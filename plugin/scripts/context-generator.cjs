...
`+JSON.stringify(o,null,2)}catch{u=" "+this.formatData(o)}else u=" "+this.formatData(o);let m="";if(n){let{sessionId:T,memorySessionId:N,correlationId:R,...A}=n;Object.keys(A).length>0&&(m=` {${Object.entries(A).map(([C,x])=>`${C}=${x}`).join(", ")}}`)}let l=`[${i}] [${a}] [${d}] ${_}${s}${m}${u}`;if(this.logFilePath)try{(0,U.appendFileSync)(this.logFilePath,l+`
`,"utf8")}catch(T){ce(`[LOGGER] Failed to write to log file: ${T instanceof Error?T.message:String(T)}
`)}else ce(l+`
  ...
  `).get(a,t,o)??void 0}var Ke=["private","claude-mem-context","system_instruction","system-instruction","persisted-output","system-reminder"],We=new RegExp(`<(${Ke.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g"),Ye=/<system-reminder>[\s\S]*?<\/system-reminder>/g,Ve=100;function Ms(r){let e=Object.fromEntries(Ke.map(n=>[n,0]));We.lastIndex=0;let t=0,s=r.replace(We,(n,o)=>(e[o]=(e[o]??0)+1,t+=1,""));return t>Ve&&c.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:t,maxAllowed:Ve,contentLength:r.length}),{stripped:s.trim(),counts:e}}function qe(r){return Ms(r).stripped}var vs=["task-notification"],Ar=new RegExp(`^\\s*<(${vs.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),Ir=256*1024;var he=4e3;function z(r){let e=r.trim(),s=qe(r).trim()||e;return s.length<=he?s:(c.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:s.length,storedLength:he}),`${s.slice(0,he-1)}\u2026`)}function Us(r,e){return{customTitle:r,platformSource:e?y(e):void 0}}var Z=class{db;redactEnabled;redactOpts;rt(e){return this.redactEnabled?be(e,this.redactOpts):e}rl(e){return this.redactEnabled?q(e,this.redactOpts):e}constructor(e=Fe){try{let t=Xe().redactSecrets;this.redactEnabled=t.enabled,this.redactOpts={entropySweep:t.entropySweep,entropyThreshold:t.entropyThreshold}}catch{this.redactEnabled=process.env.CLAUDE_MEM_REDACT_SECRETS!=="0"&&process.env.CLAUDE_MEM_REDACT_SECRETS!=="false",this.redactOpts={entropySweep:!0,entropyThreshold:4}}e instanceof H?this.db=e:(e!==":memory:"&&$e(O),this.db=new H(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA journal_size_limit = 4194304")),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(t=>t.name)}hasUniqueIndexOnColumns(e,t){return this.db.query(`PRAGMA index_list(${e})`).all().some(n=>{if(n.unique!==1)return!1;let o=this.getIndexColumns(n.name);return o.length===t.length&&o.every((i,a)=>i===t[a])})}resolvePromptSessionDbId(e,t,s){if(t!==void 0)return t;let n=s?y(s):void 0;return n?this.db.prepare(`
    ...
    `).all(e)}getAllProjects(e){let t=e?y(e):void 0,s=`
      ...
      WHERE project IS NOT NULL AND project != ''
        AND project != ?
    `,n=[ge];return t&&(s+=" AND COALESCE(platform_source, ?) = ?",n.push(E,t)),s+=" ORDER BY project ASC",this.db.prepare(s).all(...n).map(i=>i.project)}getProjectCatalog(){let e=this.db.prepare(`
      SELECT
    ...
    `).get(e,y(t))||null:this.db.prepare(`
        ...
        FROM observations
        WHERE id = ?
      `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i,type:a,concepts:d,files:_}=t,u=s==="relevance",m=u?"":`ORDER BY o.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,l=n&&!u?`LIMIT ${n}`:"",T=e.map(()=>"?").join(","),N=[...e],R=[];if(o&&(R.push("o.project = ?"),N.push(o)),i&&(R.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),N.push(y(i))),a)if(Array.isArray(a)){let I=a.map(()=>"?").join(",");R.push(`o.type IN (${I})`),N.push(...a)}else R.push("o.type = ?"),N.push(a);if(d){let I=Array.isArray(d)?d:[d],b=I.map(()=>"EXISTS (SELECT 1 FROM json_each(o.concepts) WHERE value = ?)");N.push(...I),R.push(`(${b.join(" OR ")})`)}if(_){let I=Array.isArray(_)?_:[_],b=I.map(()=>"(EXISTS (SELECT 1 FROM json_each(o.files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(o.files_modified) WHERE value LIKE ?))");I.forEach(L=>{N.push(`%${L}%`,`%${L}%`)}),R.push(`(${b.join(" OR ")})`)}let A=R.length>0?`WHERE o.id IN (${T}) AND ${R.join(" AND ")}`:`WHERE o.id IN (${T})`,C=this.db.prepare(`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions s ON s.memory_session_id = o.memory_session_id
      ${A}
      ${m}
      ${l}
    `).all(...N);if(!u)return C;let x=new Map(C.map(I=>[I.id,I])),h=e.map(I=>x.get(I)).filter(I=>!!I);return n?h.slice(0,n):h}getSummaryForSession(e,t){let s=[e],n="";return t&&(n=`
        AND EXISTS (
          SELECT 1
    ...
    `).run(d,e,t,a,o.toISOString(),i).lastInsertRowid}getUserPrompt(e,t,s){let n=this.resolvePromptSessionDbId(e,s);return n!==null?this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a){let d=i??Date.now(),_=new Date(d).toISOString(),u=this.rt(s.title),m=this.rt(s.subtitle),l=this.rt(s.narrative),T=this.rl(s.facts),N=this.rt(s.metadata??null),R=Te(e,u??null,l??null),f=this.db.prepare(`
    ...
    `).get(e,t,s.type,u,m,JSON.stringify(T),l,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,s.agent_type??null,s.agent_id??null,R,_,d,a||null,N);if(f)return{id:f.id,createdAtEpoch:f.created_at_epoch};let C=this.db.prepare("SELECT id, created_at_epoch FROM observations WHERE memory_session_id = ? AND content_hash = ?").get(e,R);if(!C)throw new Error(`storeObservation: ON CONFLICT without existing row for content_hash=${R}`);return{id:C.id,createdAtEpoch:C.created_at_epoch}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,this.rt(s.request),this.rt(s.investigated),this.rt(s.learned),this.rt(s.completed),this.rt(s.next_steps),this.rt(s.notes),n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let _=a??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, agent_type, agent_id, content_hash, created_at, created_at_epoch,
         generated_by_model)
        ...
        ON CONFLICT(memory_session_id, content_hash) DO NOTHING
        RETURNING id
      `),N=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let A of s){let f=this.rt(A.title),C=this.rt(A.subtitle),x=this.rt(A.narrative),h=this.rl(A.facts),I=Te(e,f??null,x??null),b=T.get(e,t,A.type,f,C,JSON.stringify(h),x,JSON.stringify(A.concepts),JSON.stringify(A.files_read),JSON.stringify(A.files_modified),o||null,i,A.agent_type??null,A.agent_id??null,I,u,_,d||null);if(b){l.push(b.id);continue}let L=N.get(e,I);if(!L)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${I}`);l.push(L.id)}let R=null;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,this.rt(n.request),this.rt(n.investigated),this.rt(n.learned),this.rt(n.completed),this.rt(n.next_steps),this.rt(n.notes),o||null,i,u,_);R=Number(f.lastInsertRowid)}return{observationIds:l,summaryId:R,createdAtEpoch:_}})()}deleteObservationsByProject(e,t={}){let s=(e??"").trim();if(s===""||s==="*")throw new Error(`deleteObservationsByProject: refusing unsafe project '${e}'`);let n=this.db.prepare("SELECT count(*) AS c FROM observations WHERE project = ?").get(s).c,o=this.db.prepare("SELECT count(*) AS c FROM session_summaries WHERE project = ?").get(s).c;if(t.dryRun)return{project:s,dryRun:!0,observationsDeleted:n,summariesDeleted:o};this.db.transaction(()=>{this.db.prepare("DELETE FROM observations WHERE project = ?").run(s),this.db.prepare("DELETE FROM session_summaries WHERE project = ?").run(s)})();try{this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='observations_fts'").all().length>0&&this.db.run("INSERT INTO observations_fts(observations_fts) VALUES('rebuild')")}catch(a){c.warn("DB","observations_fts rebuild after project delete failed",{project:s},a instanceof Error?a:new Error(String(a)))}return c.info("DB","Deleted observations by project",{project:s,observationsDeleted:n,summariesDeleted:o}),{project:s,dryRun:!1,observationsDeleted:n,summariesDeleted:o}}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY ss.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n&&!a?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),m=[...e],l=[];o&&(l.push("ss.project = ?"),m.push(o)),i&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),m.push(y(i)));let T=l.length>0?`AND ${l.join(" AND ")}`:"",R=this.db.prepare(`
      SELECT ss.*
      FROM session_summaries ss
      ...
      ${d}
      ${_}
    `).all(...m);if(!a)return R;let A=new Map(R.map(C=>[C.id,C])),f=e.map(C=>A.get(C)).filter(C=>!!C);return n?f.slice(0,n):f}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY up.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),m=[...e],l=[];o&&(l.push("s.project = ?"),m.push(o)),i&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),m.push(y(i)));let T=l.length>0?`AND ${l.join(" AND ")}`:"",R=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${d}
      ${_}
    `).all(...m);if(!a)return R;let A=new Map(R.map(f=>[f.id,f]));return e.map(f=>A.get(f)).filter(f=>!!f)}getTimelineAroundTimestamp(e,t=10,s=10,n,o){return this.getTimelineAroundObservation(null,e,t,s,n,o)}getTimelineAroundObservation(e,t,s=10,n=10,o,i){let a=i?y(i):void 0,d=(h,I)=>{let b=[],L=[];return o&&(b.push(`${h}.project = ?`),L.push(o)),a&&(b.push(`COALESCE(NULLIF(${I}.platform_source, ''), '${E}') = ?`),L.push(a)),{clause:b.length>0?`AND ${b.join(" AND ")}`:"",params:L}},_=d("o","src"),u=d("ss","src"),m=d("s","s"),l,T;if(e!==null){let h=`
      ...
      `,I=`
        ...
        ORDER BY o.id ASC
        LIMIT ?
      `;try{let b=this.db.prepare(h).all(e,..._.params,s+1),L=this.db.prepare(I).all(e,..._.params,n+1);if(b.length===0&&L.length===0)return{observations:[],sessions:[],prompts:[]};l=b.length>0?b[b.length-1].created_at_epoch:t,T=L.length>0?L[L.length-1].created_at_epoch:t}catch(b){return b instanceof Error?c.error("DB","Error getting boundary observations",{project:o},b):c.error("DB","Error getting boundary observations with non-Error",{},new Error(String(b))),{observations:[],sessions:[],prompts:[]}}}else{let h=`
      ...
      `,I=`
        ...
        LIMIT ?
      `;try{let b=this.db.prepare(h).all(t,..._.params,s),L=this.db.prepare(I).all(t,..._.params,n+1);if(b.length===0&&L.length===0)return{observations:[],sessions:[],prompts:[]};l=b.length>0?b[b.length-1].created_at_epoch:t,T=L.length>0?L[L.length-1].created_at_epoch:t}catch(b){return b instanceof Error?c.error("DB","Error getting boundary timestamps",{project:o},b):c.error("DB","Error getting boundary timestamps with non-Error",{},new Error(String(b))),{observations:[],sessions:[],prompts:[]}}}let N=`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions src ON src.memory_session_id = o.memory_session_id
    ...
    `,f=this.db.prepare(N).all(l,T,..._.params),C=this.db.prepare(R).all(l,T,...u.params),x=this.db.prepare(A).all(l,T,...m.params);return{observations:f,sessions:C.map(h=>({id:h.id,memory_session_id:h.memory_session_id,project:h.project,request:h.request,completed:h.completed,next_steps:h.next_steps,created_at:h.created_at,created_at_epoch:h.created_at_epoch})),prompts:x.map(h=>({id:h.id,content_session_id:h.content_session_id,prompt_number:h.prompt_number,prompt_text:h.prompt_text,project:h.project,platform_source:h.platform_source,created_at:h.created_at,created_at_epoch:h.created_at_epoch}))}}getPromptById(e){return this.db.prepare(`
      SELECT
        p.id,
...
`,"utf-8"),a;try{a=(0,S.statSync)(t).mode&511}catch{}let d;try{d=a!==void 0?(0,S.openSync)(o,"w",a):(0,S.openSync)(o,"w");let _=0;for(;_<i.length;){let u=(0,S.writeSync)(d,i,_,i.length-_);if(u===0)throw new Error(`writeSync stalled at ${_}/${i.length} bytes`);_+=u}if((0,S.fsyncSync)(d),(0,S.closeSync)(d),d=void 0,(0,S.renameSync)(o,t),!ks){let u;try{u=(0,S.openSync)(s,"r"),(0,S.fsyncSync)(u)}catch{}finally{if(u!==void 0)try{(0,S.closeSync)(u)}catch{}}}}catch(_){if(d!==void 0)try{(0,S.closeSync)(d)}catch{}try{(0,S.unlinkSync)(o)}catch{}throw _}}var re=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-haiku-4-5-20251001",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:String(37700+(process.getuid?.()??77)%100),CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_API_TIMEOUT_MS:String(st(Oe.API_REQUEST)),CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_PROVIDER:"claude",CLAUDE_MEM_CLAUDE_AUTH_METHOD:"subscription",CLAUDE_MEM_GEMINI_API_KEY:"",CLAUDE_MEM_GEMINI_MODEL:"gemini-2.5-flash-lite",CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED:"true",CLAUDE_MEM_GEMINI_MAX_CONTEXT_MESSAGES:"20",CLAUDE_MEM_GEMINI_MAX_TOKENS:"100000",CLAUDE_MEM_OPENROUTER_API_KEY:"",CLAUDE_MEM_OPENROUTER_MODEL:"xiaomi/mimo-v2-flash:free",CLAUDE_MEM_OPENROUTER_BASE_URL:"",CLAUDE_MEM_OPENROUTER_SITE_URL:"",CLAUDE_MEM_OPENROUTER_APP_NAME:"claude-mem",CLAUDE_MEM_OPENROUTER_MAX_CONTEXT_MESSAGES:"20",CLAUDE_MEM_OPENROUTER_MAX_TOKENS:"100000",CLAUDE_MEM_DATA_DIR:(0,X.join)((0,Ae.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_MODE:"code",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"false",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"false",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"false",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_FULL_COUNT:"0",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false",CLAUDE_MEM_CONTEXT_SHOW_TERMINAL_OUTPUT:"true",CLAUDE_MEM_WELCOME_HINT_ENABLED:"true",CLAUDE_MEM_FOLDER_CLAUDEMD_ENABLED:"false",CLAUDE_MEM_FOLDER_USE_LOCAL_MD:"false",CLAUDE_MEM_TRANSCRIPTS_ENABLED:"true",CLAUDE_MEM_TRANSCRIPTS_CONFIG_PATH:(0,X.join)((0,Ae.homedir)(),".claude-mem","transcript-watch.json"),CLAUDE_MEM_CODEX_TRANSCRIPT_INGESTION:"false",CLAUDE_MEM_MAX_CONCURRENT_AGENTS:"2",CLAUDE_MEM_HOOK_FAIL_LOUD_THRESHOLD:"3",CLAUDE_MEM_EXCLUDED_PROJECTS:"",CLAUDE_MEM_FOLDER_MD_EXCLUDE:"[]",CLAUDE_MEM_FOLDER_MD_SKELETON_DENYLIST:"[]",CLAUDE_MEM_SEMANTIC_INJECT:"false",CLAUDE_MEM_SEMANTIC_INJECT_LIMIT:"5",CLAUDE_MEM_TIER_ROUTING_ENABLED:"true",CLAUDE_MEM_TIER_SIMPLE_MODEL:"haiku",CLAUDE_MEM_TIER_SUMMARY_MODEL:"",CLAUDE_MEM_TIER_FAST_MODEL:"haiku",CLAUDE_MEM_TIER_SMART_MODEL:"sonnet",CLAUDE_MEM_CHROMA_ENABLED:"true",CLAUDE_MEM_CHROMA_MODE:"local",CLAUDE_MEM_CHROMA_HOST:"127.0.0.1",CLAUDE_MEM_CHROMA_PORT:"8000",CLAUDE_MEM_CHROMA_SSL:"false",CLAUDE_MEM_CHROMA_API_KEY:"",CLAUDE_MEM_CHROMA_TENANT:"default_tenant",CLAUDE_MEM_CHROMA_DATABASE:"default_database",CLAUDE_MEM_CHROMA_PREWARM_TIMEOUT_MS:"120000",CLAUDE_MEM_TELEGRAM_ENABLED:"true",CLAUDE_MEM_TELEGRAM_BOT_TOKEN:"",CLAUDE_MEM_TELEGRAM_CHAT_ID:"",CLAUDE_MEM_TELEGRAM_TRIGGER_TYPES:"security_alert",CLAUDE_MEM_TELEGRAM_TRIGGER_CONCEPTS:"",CLAUDE_MEM_QUEUE_ENGINE:"sqlite",CLAUDE_MEM_REDIS_URL:"",CLAUDE_MEM_REDIS_HOST:"127.0.0.1",CLAUDE_MEM_REDIS_PORT:"6379",CLAUDE_MEM_REDIS_MODE:"external",CLAUDE_MEM_QUEUE_REDIS_PREFIX:`claude_mem_${process.env.CLAUDE_MEM_WORKER_PORT??String(37700+(process.getuid?.()??77)%100)}`,CLAUDE_MEM_AUTH_MODE:"api-key",CLAUDE_MEM_RUNTIME:"worker",CLAUDE_MEM_SERVER_URL:`http://127.0.0.1:${process.env.CLAUDE_MEM_SERVER_PORT??String(37877+(process.getuid?.()??77)%100)}`,CLAUDE_MEM_SERVER_API_KEY:"",CLAUDE_MEM_SERVER_PROJECT_ID:"",CLAUDE_MEM_SERVER_BETA_URL:`http://127.0.0.1:${process.env.CLAUDE_MEM_SERVER_PORT??String(37877+(process.getuid?.()??77)%100)}`,CLAUDE_MEM_SERVER_BETA_API_KEY:"",CLAUDE_MEM_SERVER_BETA_PROJECT_ID:""};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return process.env[e]??this.DEFAULTS[e]}static getInt(e){let t=this.get(e);return parseInt(t,10)}static getBool(e){let t=this.get(e);return t==="true"||t===!0}static applyEnvOverrides(e){let t={...e};for(let s of Object.keys(this.DEFAULTS))process.env[s]!==void 0&&(t[s]=process.env[s]);return t}static loadFromFile(e,t=!0){try{if(!(0,v.existsSync)(e)){let a=this.getAllDefaults();try{let d=(0,X.dirname)(e);(0,v.existsSync)(d)||(0,v.mkdirSync)(d,{recursive:!0}),se(e,a),console.warn("[SETTINGS] Created settings file with defaults:",e)}catch(d){console.warn("[SETTINGS] Failed to create settings file, using in-memory defaults:",e,d instanceof Error?d.message:String(d))}return t?this.applyEnvOverrides(a):a}let s=(0,v.readFileSync)(e,"utf-8"),n=JSON.parse(s.replace(/^\uFEFF/,"")),o=n;if(n.env&&typeof n.env=="object"){o=n.env;try{se(e,o),console.warn("[SETTINGS] Migrated settings file from nested to flat schema:",e)}catch(a){console.warn("[SETTINGS] Failed to auto-migrate settings file:",e,a instanceof Error?a.message:String(a))}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))o[a]!==void 0&&(i[a]=o[a]);return t?this.applyEnvOverrides(i):i}catch(s){console.warn("[SETTINGS] Failed to load settings, using defaults:",e,s instanceof Error?s.message:String(s));let n=this.getAllDefaults();try{if((0,v.existsSync)(e)){let o=`${e}.corrupt-${Date.now()}`;(0,v.renameSync)(e,o),console.warn("[SETTINGS] Backed up corrupt settings file to:",o)}se(e,n),console.warn("[SETTINGS] Recovered settings file with defaults:",e)}catch(o){console.warn("[SETTINGS] Failed to recover corrupt settings file:",e,o instanceof Error?o.message:String(o))}return t?this.applyEnvOverrides(n):n}}};var B=require("fs"),ne=require("path");var D=class r{static instance=null;activeMode=null;modesDir;constructor(){let e=He(),t=[...process.env.CLAUDE_MEM_MODES_DIR?[process.env.CLAUDE_MEM_MODES_DIR]:[],(0,ne.join)(e,"modes"),(0,ne.join)(e,"..","plugin","modes")],s=t.find(n=>(0,B.existsSync)(n));this.modesDir=s||t[0]}static getInstance(){return r.instance||(r.instance=new r),r.instance}parseInheritance(e){let t=e.split("--");if(t.length===1)return{hasParent:!1,parentId:"",overrideId:""};if(t.length>2)throw new Error(`Invalid mode inheritance: ${e}. Only one level of inheritance supported (parent--override)`);return{hasParent:!0,parentId:t[0],overrideId:e}}isPlainObject(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}deepMerge(e,t){let s={...e};for(let n in t){let o=t[n],i=e[n];this.isPlainObject(o)&&this.isPlainObject(i)?s[n]=this.deepMerge(i,o):s[n]=o}return s}loadModeFile(e){let t=(0,ne.join)(this.modesDir,`${e}.json`);if(!(0,B.existsSync)(t))throw new Error(`Mode file not found: ${t}`);let s=(0,B.readFileSync)(t,"utf-8");return JSON.parse(s)}loadMode(e){let t=this.parseInheritance(e);if(!t.hasParent)try{let d=this.loadModeFile(e);return this.activeMode=d,c.debug("SYSTEM",`Loaded mode: ${d.name} (${e})`,void 0,{types:d.observation_types.map(_=>_.id),concepts:d.observation_concepts.map(_=>_.id)}),d}catch(d){if(d instanceof Error?c.warn("WORKER",`Mode file not found: ${e}, falling back to 'code'`,{message:d.message}):c.warn("WORKER",`Mode file not found: ${e}, falling back to 'code'`,{error:String(d)}),e==="code")throw new Error("Critical: code.json mode file missing");return this.loadMode("code")}let{parentId:s,overrideId:n}=t,o;try{o=this.loadMode(s)}catch(d){d instanceof Error?c.warn("WORKER",`Parent mode '${s}' not found for ${e}, falling back to 'code'`,{message:d.message}):c.warn("WORKER",`Parent mode '${s}' not found for ${e}, falling back to 'code'`,{error:String(d)}),o=this.loadMode("code")}let i;try{i=this.loadModeFile(n),c.debug("SYSTEM",`Loaded override file: ${n} for parent ${s}`)}catch(d){return d instanceof Error?c.warn("WORKER",`Override file '${n}' not found, using parent mode '${s}' only`,{message:d.message}):c.warn("WORKER",`Override file '${n}' not found, using parent mode '${s}' only`,{error:String(d)}),this.activeMode=o,o}if(!i)return c.warn("SYSTEM",`Invalid override file: ${n}, using parent mode '${s}' only`),this.activeMode=o,o;let a=this.deepMerge(o,i);return this.activeMode=a,c.debug("SYSTEM",`Loaded mode with inheritance: ${a.name} (${e} = ${s} + ${n})`,void 0,{parent:s,override:n,types:a.observation_types.map(d=>d.id),concepts:a.observation_concepts.map(d=>d.id)}),a}getActiveMode(){if(!this.activeMode)throw new Error("No mode loaded. Call loadMode() first.");return this.activeMode}getObservationTypes(){return this.getActiveMode().observation_types}getTypeIcon(e){return this.getObservationTypes().find(s=>s.id===e)?.emoji||"\u{1F4DD}"}getWorkEmoji(e){return this.getObservationTypes().find(s=>s.id===e)?.work_emoji||"\u{1F4DD}"}};function nt(){let r=k.settings(),e=re.loadFromFile(r),t=D.getInstance().getActiveMode(),s=new Set(t.observation_types.map(o=>o.id)),n=new Set(t.observation_concepts.map(o=>o.id));return{totalObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_OBSERVATIONS,10),fullObservationCount:parseInt(e.CLAUDE_MEM_CONTEXT_FULL_COUNT,10),sessionCount:parseInt(e.CLAUDE_MEM_CONTEXT_SESSION_COUNT,10),showReadTokens:e.CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS==="true",showWorkTokens:e.CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS==="true",showSavingsAmount:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT==="true",showSavingsPercent:e.CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT==="true",observationTypes:s,observationConcepts:n,fullObservationField:e.CLAUDE_MEM_CONTEXT_FULL_FIELD,showLastSummary:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY==="true",showLastMessage:e.CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE==="true"}}var p={reset:"\x1B[0m",bright:"\x1B[1m",dim:"\x1B[2m",cyan:"\x1B[36m",green:"\x1B[32m",yellow:"\x1B[33m",blue:"\x1B[34m",magenta:"\x1B[35m",gray:"\x1B[90m",red:"\x1B[31m"},ot=4,Ie=1;function it(r){let e=(r.title?.length||0)+(r.subtitle?.length||0)+(r.narrative?.length||0)+JSON.stringify(r.facts||[]).length;return Math.ceil(e/ot)}function Ne(r){let e=r.length,t=r.reduce((i,a)=>i+it(a),0),s=r.reduce((i,a)=>i+(a.discovery_tokens||0),0),n=s-t,o=s>0?Math.round(n/s*100):0;return{totalObservations:e,totalReadTokens:t,totalDiscoveryTokens:s,savings:n,savingsPercent:o}}function Ps(r){return D.getInstance().getWorkEmoji(r)}function j(r,e){let t=it(r),s=r.discovery_tokens||0,n=Ps(r.type),o=s>0?`${n} ${s.toLocaleString()}`:"-";return{readTokens:t,discoveryTokens:s,discoveryDisplay:o,workEmoji:n}}function oe(r){return r.showReadTokens||r.showWorkTokens||r.showSavingsAmount||r.showSavingsPercent}var at=$(require("path"),1),ie=require("fs");function dt(r,e,t,s){let n=Array.from(t.observationTypes),o=n.map(()=>"?").join(","),i=Array.from(t.observationConcepts),a=i.map(()=>"?").join(",");return r.db.prepare(`
  ...
  `).all(...e,...e,s??null,s??null,...n,...i,t.totalObservationCount)}function ct(r,e,t,s){let n=e.map(()=>"?").join(",");return r.db.prepare(`
    ...
    ORDER BY ss.created_at_epoch DESC
    LIMIT ?
  `).all(...e,...e,s??null,s??null,t.sessionCount+Ie)}function $s(r){return r.replace(/[/.]/g,"-")}function Hs(r){if(!r.includes('"type":"assistant"'))return null;let e=JSON.parse(r);if(e.type==="assistant"&&e.message?.content&&Array.isArray(e.message.content)){let t="";for(let s of e.message.content)s.type==="text"&&(t+=s.text);if(t=t.replace(Ye,"").trim(),t)return t}return null}function Gs(r){for(let e=r.length-1;e>=0;e--)try{let t=Hs(r[e]);if(t)return t}catch(t){t instanceof Error?c.debug("WORKER","Skipping malformed transcript line",{lineIndex:e},t):c.debug("WORKER","Skipping malformed transcript line",{lineIndex:e,error:String(t)});continue}return""}function Xs(r){try{if(!(0,ie.existsSync)(r))return{assistantMessage:""};let e=(0,ie.readFileSync)(r,"utf-8").trim();if(!e)return{assistantMessage:""};let t=e.split(`
`).filter(n=>n.trim());return{assistantMessage:Gs(t)}}catch(e){return e instanceof Error?c.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:r},e):c.warn("WORKER","Failed to extract prior messages from transcript",{transcriptPath:r,error:String(e)}),{assistantMessage:""}}}function pt(r,e,t,s){if(!e.showLastMessage||r.length===0)return{assistantMessage:""};let n=r.find(d=>d.memory_session_id!==t);if(!n)return{assistantMessage:""};let o=n.memory_session_id,i=$s(s),a=at.default.join(P,"projects",i,`${o}.jsonl`);return Xs(a)}function lt(r,e){let t=e[0]?.id;return r.map((s,n)=>{let o=n===0?null:e[n+1];return{...s,displayEpoch:o?o.created_at_epoch:s.created_at_epoch,displayTime:o?o.created_at:s.created_at,shouldShowLink:s.id!==t}})}function Et(r,e){let t=[...r.map(s=>({type:"observation",data:s})),...e.map(s=>({type:"summary",data:s}))];return t.sort((s,n)=>{let o=s.type==="observation"?s.data.created_at_epoch:s.data.displayEpoch,i=n.type==="observation"?n.data.created_at_epoch:n.data.displayEpoch;return o-i}),t}function mt(r,e){return new Set(r.slice(0,e).map(t=>t.id))}function gt(){let r=new Date,e=r.toLocaleDateString("en-CA"),t=r.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),s=r.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${s}`}function Tt(r){return[`# [${r}] recent context, ${gt()}`,""]}function ft(){return[`Legend: \u{1F3AF}session ${D.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji}${t.id}`).join(" ")}`,"Format: ID TIME TYPE TITLE","Fetch details: get_observations([IDs]) | Search: mem-search skill",""]}function St(r,e){let t=[],s=[`${r.totalObservations} obs (${r.totalReadTokens.toLocaleString()}t read)`,`${r.totalDiscoveryTokens.toLocaleString()}t work`];return r.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)&&(e.showSavingsPercent?s.push(`${r.savingsPercent}% savings`):e.showSavingsAmount&&s.push(`${r.savings.toLocaleString()}t saved`)),t.push(`Stats: ${s.join(" | ")}`),t.push(""),t}function bt(r){return[`### ${r}`]}function Rt(r){return r.toLowerCase().replace(" am","a").replace(" pm","p")}function ht(r,e,t){let s=r.title||"Untitled",n=D.getInstance().getTypeIcon(r.type),o=e?Rt(e):'"';return`${r.id} ${o} ${n} ${s}`}function Ot(r,e,t,s){let n=[],o=r.title||"Untitled",i=D.getInstance().getTypeIcon(r.type),a=e?Rt(e):'"',{readTokens:d,discoveryDisplay:_}=j(r,s);n.push(`**${r.id}** ${a} ${i} **${o}**`),t&&n.push(t);let u=[];return s.showReadTokens&&u.push(`~${d}t`),s.showWorkTokens&&u.push(_),u.length>0&&n.push(u.join(" ")),n.push(""),n}function At(r,e){return[`S${r.id} ${r.request||"Session started"} (${e})`]}function W(r,e){return e?[`**${r}**: ${e}`,""]:[]}function It(r){return r.assistantMessage?["","---","","**Previously**","",`A: ${r.assistantMessage}`,""]:[]}function Nt(r,e){return["",`Access ${Math.round(r/1e3)}k tokens of past work via get_observations([IDs]) or mem-search skill.`]}function Ct(r){return`# [${r}] recent context, ${gt()}

No previous sessions found.`}function Lt(){let r=new Date,e=r.toLocaleDateString("en-CA"),t=r.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}).toLowerCase().replace(" ",""),s=r.toLocaleTimeString("en-US",{timeZoneName:"short"}).split(" ").pop();return`${e} ${t} ${s}`}function yt(r){return["",`${p.bright}${p.cyan}[${r}] recent context, ${Lt()}${p.reset}`,`${p.gray}${"\u2500".repeat(60)}${p.reset}`,""]}function Dt(){let e=D.getInstance().getActiveMode().observation_types.map(t=>`${t.emoji} ${t.id}`).join(" | ");return[`${p.dim}Legend: session-request | ${e}${p.reset}`,""]}function Mt(){return[`${p.bright}Column Key${p.reset}`,`${p.dim}  Read: Tokens to read this observation (cost to learn it now)${p.reset}`,`${p.dim}  Work: Tokens spent on work that produced this record ( research, building, deciding)${p.reset}`,""]}function vt(){return[`${p.dim}Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${p.reset}`,"",`${p.dim}When you need implementation details, rationale, or debugging context:${p.reset}`,`${p.dim}  - Fetch by ID: get_observations([IDs]) for observations visible in this index${p.reset}`,`${p.dim}  - Search history: Use the mem-search skill for past decisions, bugs, and deeper research${p.reset}`,`${p.dim}  - Trust this index over re-reading code for past decisions and learnings${p.reset}`,""]}function Ut(r,e){let t=[];if(t.push(`${p.bright}${p.cyan}Context Economics${p.reset}`),t.push(`${p.dim}  Loading: ${r.totalObservations} observations (${r.totalReadTokens.toLocaleString()} tokens to read)${p.reset}`),t.push(`${p.dim}  Work investment: ${r.totalDiscoveryTokens.toLocaleString()} tokens spent on research, building, and decisions${p.reset}`),r.totalDiscoveryTokens>0&&(e.showSavingsAmount||e.showSavingsPercent)){let s="  Your savings: ";e.showSavingsAmount&&e.showSavingsPercent?s+=`${r.savings.toLocaleString()} tokens (${r.savingsPercent}% reduction from reuse)`:e.showSavingsAmount?s+=`${r.savings.toLocaleString()} tokens`:s+=`${r.savingsPercent}% reduction from reuse`,t.push(`${p.green}${s}${p.reset}`)}return t.push(""),t}function xt(r){return[`${p.bright}${p.cyan}${r}${p.reset}`,""]}function wt(r){return[`${p.dim}${r}${p.reset}`]}function kt(r,e,t,s){let n=r.title||"Untitled",o=D.getInstance().getTypeIcon(r.type),{readTokens:i,discoveryTokens:a,workEmoji:d}=j(r,s),_=t?`${p.dim}${e}${p.reset}`:" ".repeat(e.length),u=s.showReadTokens&&i>0?`${p.dim}(~${i}t)${p.reset}`:"",m=s.showWorkTokens&&a>0?`${p.dim}(${d} ${a.toLocaleString()}t)${p.reset}`:"";return`  ${p.dim}#${r.id}${p.reset}  ${_}  ${o}  ${n} ${u} ${m}`}function Ft(r,e,t,s,n){let o=[],i=r.title||"Untitled",a=D.getInstance().getTypeIcon(r.type),{readTokens:d,discoveryTokens:_,workEmoji:u}=j(r,n),m=t?`${p.dim}${e}${p.reset}`:" ".repeat(e.length),l=n.showReadTokens&&d>0?`${p.dim}(~${d}t)${p.reset}`:"",T=n.showWorkTokens&&_>0?`${p.dim}(${u} ${_.toLocaleString()}t)${p.reset}`:"";return o.push(`  ${p.dim}#${r.id}${p.reset}  ${m}  ${a}  ${p.bright}${i}${p.reset}`),s&&o.push(`    ${p.dim}${s}${p.reset}`),(l||T)&&o.push(`    ${l} ${T}`),o.push(""),o}function Pt(r,e){let t=`${r.request||"Session started"} (${e})`;return[`${p.yellow}#S${r.id}${p.reset} ${t}`,""]}function V(r,e,t){return e?[`${t}${r}:${p.reset} ${e}`,""]:[]}function $t(r){return r.assistantMessage?["","---","",`${p.bright}${p.magenta}Previously${p.reset}`,"",`${p.dim}A: ${r.assistantMessage}${p.reset}`,""]:[]}function Ht(r,e){let t=Math.round(r/1e3);return["",`${p.dim}Access ${t}k tokens of past research & decisions for just ${e.toLocaleString()}t. Use the claude-mem skill to access memories by ID.${p.reset}`]}function Gt(r){return`
${p.bright}${p.cyan}[${r}] recent context, ${Lt()}${p.reset}
${p.gray}${"\u2500".repeat(60)}${p.reset}

${p.dim}No previous sessions found for this project yet.${p.reset}
`}function Xt(r,e,t,s){let n=[];return s?n.push(...yt(r)):n.push(...Tt(r)),s?n.push(...Dt()):n.push(...ft()),s&&(n.push(...Mt()),n.push(...vt())),oe(t)&&(s?n.push(...Ut(e,t)):n.push(...St(e,t))),n}var Ce=$(require("path"),1);function _e(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e instanceof Error?e:new Error(String(e))),[]}}function Le(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ye(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function jt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function Bt(r,e){return Ce.default.isAbsolute(r)?Ce.default.relative(e,r):r}function Wt(r,e,t){let s=_e(r);if(s.length>0)return Bt(s[0],e);if(t){let n=_e(t);if(n.length>0)return Bt(n[0],e)}return"General"}function Bs(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=jt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Vt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?_e(r.facts).join(`
`):null}function js(r,e,t,s){let n=[];n.push(...bt(r));let o="";for(let i of e)if(i.type==="summary"){let a=i.data,d=Le(a.displayTime);n.push(...At(a,d))}else{let a=i.data,d=ye(a.created_at),u=d!==o?d:"";if(o=d,t.has(a.id)){let l=Vt(a,s);n.push(...Ot(a,u,l,s))}else n.push(ht(a,u,s))}return n}function Ws(r,e,t,s,n){let o=[];o.push(...xt(r));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let _=d.data,u=Le(_.displayTime);o.push(...Pt(_,u))}else{let _=d.data,u=Wt(_.files_modified,n,_.files_read),m=ye(_.created_at),l=m!==a;a=m;let T=t.has(_.id);if(u!==i&&(o.push(...wt(u)),i=u),T){let N=Vt(_,s);o.push(...Ft(_,m,l,N,s))}else o.push(kt(_,m,l,s))}return o.push(""),o}function Vs(r,e,t,s,n,o){return o?Ws(r,e,t,s,n):js(r,e,t,s)}function Kt(r,e,t,s,n){let o=[],i=Bs(r);for(let[a,d]of i)o.push(...Vs(a,d,e,t,s,n));return o}function Yt(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function qt(r,e){let t=[];return e?(t.push(...V("Investigated",r.investigated,p.blue)),t.push(...V("Learned",r.learned,p.yellow)),t.push(...V("Completed",r.completed,p.green)),t.push(...V("Next Steps",r.next_steps,p.magenta))):(t.push(...W("Investigated",r.investigated)),t.push(...W("Learned",r.learned)),t.push(...W("Completed",r.completed)),t.push(...W("Next Steps",r.next_steps))),t}function Jt(r,e){return e?$t(r):It(r)}function Qt(r,e,t){return!oe(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Ht(r.totalDiscoveryTokens,r.totalReadTokens):Nt(r.totalDiscoveryTokens,r.totalReadTokens)}var Ks=zt.default.join((0,Zt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Ys(){try{return new Z}catch(r){if(r instanceof Error&&r.code==="ERR_DLOPEN_FAILED"){try{(0,es.unlinkSync)(Ks)}catch(e){e instanceof Error?c.debug("WORKER","Marker file cleanup failed (may not exist)",{},e):c.debug("WORKER","Marker file cleanup failed (may not exist)",{error:String(e)})}return c.error("WORKER","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function qs(r,e){return e?Gt(r):Ct(r)}function Js(r,e,t,s,n,o,i){let a=[],d=Ne(e);a.push(...Xt(r,d,s,i));let _=t.slice(0,s.sessionCount),u=lt(_,t),m=Et(e,u),l=mt(e,s.fullObservationCount);a.push(...Kt(m,l,s,n,i));let T=t[0],N=e[0];Yt(s,T,N)&&a.push(...qt(T,i));let R=pt(e,s,o,n);return a.push(...Jt(R,i)),a.push(...Qt(d,s,i)),a.join(`
