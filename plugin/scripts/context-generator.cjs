...
`).map(r=>r.trim()).find(r=>r.length>0)??"Session checkpoint").replace(/^#+\s*/,"").replace(/^\*+\s*/,"").replace(/\*+$/,"").trim();return t?t.length>80?`${t.slice(0,80).trimEnd()}\u2026`:t:"Session checkpoint"}var Zs=/^0\d{3}$/,en=/^V-\d{4}$/;function tn(s){let e=s?`${s}.`:"";return`COALESCE(json_extract(${e}metadata, '$.record_id'), json_extract(${e}metadata, '$.vorgang_id'))`}var w=tn();function rn(s){let e=s.trim();return Zs.test(e)?"akte":en.test(e)?"vorgang":null}function Ft(s,e){if(s)try{if(JSON.parse(s)?.kind==="vorgang")return"vorgang"}catch{}return(e&&rn(e))==="vorgang"?"vorgang":"akte"}var $t="keepmind://curated/";var B="revised_by";function on(s,e){return{customTitle:s,platformSource:e?D(e):void 0}}var he=class{db;redactEnabled;redactOpts;mq;rt(e){return this.redactEnabled?Ve(e,this.redactOpts):e}rl(e){return this.redactEnabled?_e(e,this.redactOpts):e}constructor(e=j){try{this.mq=ge();let t=this.mq.redactSecrets;this.redactEnabled=t.enabled,this.redactOpts={entropySweep:t.entropySweep,entropyThreshold:t.entropyThreshold}}catch{this.mq=qe,this.redactEnabled=y("KEEPMIND_REDACT_SECRETS")!=="0"&&y("KEEPMIND_REDACT_SECRETS")!=="false",this.redactOpts={entropySweep:!0,entropyThreshold:4}}if(e instanceof J)this.db=e;else{e!==":memory:"&&ht(R);let t=e===j?Z():e;this.db=new J(t),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run(`PRAGMA journal_size_limit = ${4194304}`),this.db.run(`PRAGMA busy_timeout = ${5e3}`)}this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex(),this.addObservationImportanceColumn(),this.addObservationBitemporalColumns(),this.addObservationLastUsedColumn(),this.addObservationUsageChannelColumns(),this.recomputeSubjectKeys(),this.addCuratedSourceColumns(),this.createDecisionEdgesTable()}createDecisionEdgesTable(){let e=this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(42),t=this.db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='decision_edges'").all();e&&t.length>0||(this.db.run(`
    ...
    `).get(e).count}createSDKSession(e,t,r,n,o){let i=new Date,a=i.getTime(),d=on(n,o),c=d.platformSource??E,l=this.rt(be(r)),_=this.db.prepare(`
