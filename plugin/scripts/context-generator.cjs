...
`+JSON.stringify(o,null,2)}catch{u=" "+this.formatData(o)}else u=" "+this.formatData(o);let l="";if(n){let{sessionId:g,memorySessionId:N,correlationId:A,...R}=n;Object.keys(R).length>0&&(l=` {${Object.entries(R).map(([L,x])=>`${L}=${x}`).join(", ")}}`)}let p=`[${i}] [${a}] [${d}] ${_}${s}${l}${u}`;if(this.logFilePath)try{(0,U.appendFileSync)(this.logFilePath,p+`
  ...
  `).get(a,t,o)??void 0}var $e=["private","claude-mem-context","system_instruction","system-instruction","persisted-output","system-reminder"],Fe=new RegExp(`<(${$e.join("|")})\\b[^>]*>[\\s\\S]*?</\\1>`,"g"),He=/<system-reminder>[\s\S]*?<\/system-reminder>/g,Pe=100;function ls(r){let e=Object.fromEntries($e.map(n=>[n,0]));Fe.lastIndex=0;let t=0,s=r.replace(Fe,(n,o)=>(e[o]=(e[o]??0)+1,t+=1,""));return t>Pe&&E.warn("SYSTEM","tag count exceeds limit",void 0,{tagCount:t,maxAllowed:Pe,contentLength:r.length}),{stripped:s.trim(),counts:e}}function Ge(r){return ls(r).stripped}var Ts=["task-notification"],nr=new RegExp(`^\\s*<(${Ts.join("|")})\\b[^>]*>(?:(?!<\\1\\b|</\\1\\b)[\\s\\S])*</\\1>\\s*$`),or=256*1024;var le=4e3;function K(r){let e=r.trim(),s=Ge(r).trim()||e;return s.length<=le?s:(E.debug("DB","Truncated stored prompt text to the configured cap",{originalLength:s.length,storedLength:le}),`${s.slice(0,le-1)}\u2026`)}function gs(r,e){return{customTitle:r,platformSource:e?C(e):void 0}}var q=class{db;constructor(e=Me){e instanceof P?this.db=e:(e!==":memory:"&&ve(O),this.db=new P(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.db.run("PRAGMA journal_size_limit = 4194304")),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(t=>t.name)}hasUniqueIndexOnColumns(e,t){return this.db.query(`PRAGMA index_list(${e})`).all().some(n=>{if(n.unique!==1)return!1;let o=this.getIndexColumns(n.name);return o.length===t.length&&o.every((i,a)=>i===t[a])})}resolvePromptSessionDbId(e,t,s){if(t!==void 0)return t;let n=s?C(s):void 0;return n?this.db.prepare(`
    ...
    `).all(e)}getAllProjects(e){let t=e?C(e):void 0,s=`
      ...
      WHERE project IS NOT NULL AND project != ''
        AND project != ?
    `,n=[Ee];return t&&(s+=" AND COALESCE(platform_source, ?) = ?",n.push(m,t)),s+=" ORDER BY project ASC",this.db.prepare(s).all(...n).map(i=>i.project)}getProjectCatalog(){let e=this.db.prepare(`
      SELECT
    ...
    `).get(m,_,e);if(l)return t&&this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,_,u,d.customTitle||null,i.toISOString(),a);return Number(p.lastInsertRowid)}saveUserPrompt(e,t,s,n){let o=new Date,i=o.getTime(),a=K(s),d=this.resolvePromptSessionDbId(e,n);return this.db.prepare(`
    ...
    `).run(d,e,t,a,o.toISOString(),i).lastInsertRowid}getUserPrompt(e,t,s){let n=this.resolvePromptSessionDbId(e,s);return n!==null?this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i,a){let d=i??Date.now(),_=new Date(d).toISOString(),u=pe(e,s.title,s.narrative),p=this.db.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let _=a??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let p=[],g=this.db.prepare(`
        ...
        ON CONFLICT(memory_session_id, content_hash) DO NOTHING
        RETURNING id
      `),N=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let R of s){let S=pe(e,R.title,R.narrative),L=g.get(e,t,R.type,R.title,R.subtitle,JSON.stringify(R.facts),R.narrative,JSON.stringify(R.concepts),JSON.stringify(R.files_read),JSON.stringify(R.files_modified),o||null,i,R.agent_type??null,R.agent_id??null,S,u,_,d||null);if(L){p.push(L.id);continue}let x=N.get(e,S);if(!x)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${S}`);p.push(x.id)}let A=null;if(n){let S=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,u,_);A=Number(S.lastInsertRowid)}return{observationIds:p,summaryId:A,createdAtEpoch:_}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY ss.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n&&!a?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),l=[...e],p=[];o&&(p.push("ss.project = ?"),l.push(o)),i&&(p.push(`COALESCE(NULLIF(s.platform_source, ''), '${m}') = ?`),l.push(C(i)));let g=p.length>0?`AND ${p.join(" AND ")}`:"",A=this.db.prepare(`
      SELECT ss.*
      FROM session_summaries ss
      ...
      ${d}
      ${_}
    `).all(...l);if(!a)return A;let R=new Map(A.map(L=>[L.id,L])),S=e.map(L=>R.get(L)).filter(L=>!!L);return n?S.slice(0,n):S}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o,platformSource:i}=t,a=s==="relevance",d=a?"":`ORDER BY up.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=n?`LIMIT ${n}`:"",u=e.map(()=>"?").join(","),l=[...e],p=[];o&&(p.push("s.project = ?"),l.push(o)),i&&(p.push(`COALESCE(NULLIF(s.platform_source, ''), '${m}') = ?`),l.push(C(i)));let g=p.length>0?`AND ${p.join(" AND ")}`:"",A=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${d}
      ${_}
    `).all(...l);if(!a)return A;let R=new Map(A.map(S=>[S.id,S]));return e.map(S=>R.get(S)).filter(S=>!!S)}getTimelineAroundTimestamp(e,t=10,s=10,n,o){return this.getTimelineAroundObservation(null,e,t,s,n,o)}getTimelineAroundObservation(e,t,s=10,n=10,o,i){let a=i?C(i):void 0,d=(b,I)=>{let h=[],D=[];return o&&(h.push(`${b}.project = ?`),D.push(o)),a&&(h.push(`COALESCE(NULLIF(${I}.platform_source, ''), '${m}') = ?`),D.push(a)),{clause:h.length>0?`AND ${h.join(" AND ")}`:"",params:D}},_=d("o","src"),u=d("ss","src"),l=d("s","s"),p,g;if(e!==null){let b=`
    ...
    `).all(...e)}getOrCreateManualSession(e){let t=`manual-${e}`,s=`manual-content-${e}`;if(this.db.prepare("SELECT memory_session_id FROM sdk_sessions WHERE memory_session_id = ?").get(t))return t;let o=new Date;return this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'active')
    `).run(t,s,e,m,o.toISOString(),o.getTime()),E.info("SESSION","Created manual session",{memorySessionId:t,project:e}),t}close(){this.db.close()}importSdkSession(e){let t=C(e.platform_source),s=this.db.prepare(`SELECT id FROM sdk_sessions
...
`):null}function Ls(r,e,t,s){let n=[];n.push(...Et(r));let o="";for(let i of e)if(i.type==="summary"){let a=i.data,d=be(a.displayTime);n.push(...Tt(a,d))}else{let a=i.data,d=he(a.created_at),u=d!==o?d:"";if(o=d,t.has(a.id)){let p=Pt(a,s);n.push(...lt(a,u,p,s))}else n.push(mt(a,u,s))}return n}function Ds(r,e,t,s,n){let o=[];o.push(...It(r));let i=null,a="";for(let d of e)if(d.type==="summary"){i=null,a="";let _=d.data,u=be(_.displayTime);o.push(...Mt(_,u))}else{let _=d.data,u=Ft(_.files_modified,n,_.files_read),l=he(_.created_at),p=l!==a;a=l;let g=t.has(_.id);if(u!==i&&(o.push(...Ct(u)),i=u),g){let N=Pt(_,s);o.push(...Dt(_,l,p,N,s))}else o.push(Lt(_,l,p,s))}return o.push(""),o}function Ms(r,e,t,s,n,o){return o?Ds(r,e,t,s,n):Ls(r,e,t,s)}function $t(r,e,t,s,n){let o=[],i=Cs(r);for(let[a,d]of i)o.push(...Ms(a,d,e,t,s,n));return o}function Ht(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function Gt(r,e){let t=[];return e?(t.push(...W("Investigated",r.investigated,c.blue)),t.push(...W("Learned",r.learned,c.yellow)),t.push(...W("Completed",r.completed,c.green)),t.push(...W("Next Steps",r.next_steps,c.magenta))):(t.push(...j("Investigated",r.investigated)),t.push(...j("Learned",r.learned)),t.push(...j("Completed",r.completed)),t.push(...j("Next Steps",r.next_steps))),t}function Xt(r,e){return e?yt(r):gt(r)}function Bt(r,e,t){return!te(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?vt(r.totalDiscoveryTokens,r.totalReadTokens):ft(r.totalDiscoveryTokens,r.totalReadTokens)}var ys=jt.default.join((0,Wt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function vs(){try{return new q}catch(r){if(r instanceof Error&&r.code==="ERR_DLOPEN_FAILED"){try{(0,Vt.unlinkSync)(ys)}catch(e){e instanceof Error?E.debug("WORKER","Marker file cleanup failed (may not exist)",{},e):E.debug("WORKER","Marker file cleanup failed (may not exist)",{error:String(e)})}return E.error("WORKER","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Us(r,e){return e?Ut(r):St(r)}function xs(r,e,t,s,n,o,i){let a=[],d=Se(e);a.push(...xt(r,d,s,i));let _=t.slice(0,s.sessionCount),u=ot(_,t),l=it(e,u),p=at(e,s.fullObservationCount);a.push(...$t(l,p,s,n,i));let g=t[0],N=e[0];Ht(s,g,N)&&a.push(...Gt(g,i));let A=nt(e,s,o,n);return a.push(...Xt(A,i)),a.push(...Bt(d,s,i)),a.join(`
