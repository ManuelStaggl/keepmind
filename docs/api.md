...
When `KEEPMIND_AUTH_MODE=api-key`, send `Authorization: Bearer <key>`. Read endpoints require `memories:read`; write endpoints require `memories:write`.
...
- `KEEPMIND_RATE_LIMIT_PER_MIN` — max requests per API key per minute. Over the
  limit returns `429` with `Retry-After` (and `X-RateLimit-*` headers). Fail-open.
- `KEEPMIND_MONTHLY_REQUEST_CAP` — max requests per team per calendar month
  (UTC). At the cap, returns `402 quota_exceeded`. Fail-open.
- `KEEPMIND_MONTHLY_TOKEN_CAP` — max provider tokens per team per month. Gates
...
- `KEEPMIND_USAGE_METERING=1` — record one `request` usage event per
...
- `GET /v1/connect` (read scope) returns the same command with a `<YOUR_API_KEY>`
  placeholder (a GET never mints). `mcpUrl` is built from `KEEPMIND_PUBLIC_URL`
