...
| `CLAUDE_MEM_DATA_DIR`         | `~/.keepmind`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
...
| Setting                       | Default                         | Description                           |
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_DATA_DIR`         | `~/.keepmind`                 | Data directory location               |
...
1. Open the viewer at the worker URL (default `http://127.0.0.1:<worker-port>`; the active port is the value of `CLAUDE_MEM_WORKER_PORT` in `~/.keepmind/settings.json`)
