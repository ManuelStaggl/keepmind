...
description: Complete reference for integrating keepmind worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Target Audience:** Developers building keepmind integrations (VSCode extensions, IDE plugins, CLI tools).
</Note>
...
// or whatever the user set via KEEPMIND_WORKER_PORT / settings.json. Read it from
// process.env.KEEPMIND_WORKER_PORT, then ~/.keepmind/settings.json
// (KEEPMIND_WORKER_PORT key), then fall back to the deterministic default.
...
```bash
KEEPMIND_MODEL=claude-haiku-4-5-20251001  # Default Claude model for observations/summaries
KEEPMIND_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
KEEPMIND_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
KEEPMIND_DATA_DIR=                        # Optional override for the data directory
...
**Tag Stripping:** Removes `<private>` and `<keepmind-context>` tags before storage.
...
```json Response
{
  "projects": ["keepmind", "other-project", ...]
}
```
...
// Register tool
vscode.lm.registerTool('keepmind-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('keepmind', async (request, context, stream, token) => {
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('keepmind worker is not running or unreachable');
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, KEEPMIND_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('keepmind-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://github.com/ManuelStaggl/keepmind">
    Complete keepmind documentation
  </Card>
