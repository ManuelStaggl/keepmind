...
Knowledge agents let you compile a slice of your keepmind observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.

<Warning>
  **The corpus tools are opt-in.** `build_corpus`, `list_corpora`,
  `prime_corpus`, `reprime_corpus`, `query_corpus` and `rebuild_corpus` are not
  listed to the model unless `KEEPMIND_MCP_CORPUS_TOOLS` is set to the literal
  string `true` in `~/.keepmind/settings.json`. Their schemas cost 2,720
  characters of every session's context, and a corpus has to be built by hand
  before any of them means anything — so nothing on this page happens by
  default. See [Configuration](/configuration#optional-mcp-tool-groups).
</Warning>
...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="keepmind" limit=200
```
...
Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.keepmind/corpora/hooks-expertise.corpus.json`.
...
- **Model from settings**: Reads `KEEPMIND_MODEL` from user settings -- no hardcoded model IDs
