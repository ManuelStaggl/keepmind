...
description: Use keepmind memory search in Claude Desktop with MCP tools
icon: desktop
---

<Note>
**Availability:** keepmind MCP tools work with Claude Desktop on macOS and Windows.
</Note>

## Overview

Claude Desktop can access your keepmind memory database through **MCP tools**. This allows you to search past sessions, decisions, and observations directly from Claude Desktop conversations.

## Prerequisites

Before configuring MCP tools, ensure:

1. **keepmind is installed** and the worker service is running
...
1. Ensure keepmind has recorded sessions (check http://localhost:37777)
2. Verify the database exists: `ls ~/.keepmind/keepmind.db`
