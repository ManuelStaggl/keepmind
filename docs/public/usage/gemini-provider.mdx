...
keepmind supports Google's Gemini API as an alternative to the Claude Agent SDK for extracting observations from your sessions.
...
lists from the model rather than from the hook payload.

That inverts the usual reason for choosing it. Per session it now issues far
more requests, and each carries far more input, than the Claude path — a free
tier does not necessarily come out ahead once rate limits and retries are
counted. See [Configuration → Alternate providers](../configuration) for the
capability table.
...
|---------|--------|---------|-------------|
| `KEEPMIND_PROVIDER` | `claude`, `gemini` | `claude` | AI provider for observation extraction |
| `KEEPMIND_GEMINI_API_KEY` | string | — | Your Gemini API key |
| `KEEPMIND_GEMINI_MODEL` | `gemini-2.5-flash-lite`, `gemini-2.5-flash`, `gemini-3-flash-preview` | `gemini-2.5-flash-lite` | Gemini model to use |
| `KEEPMIND_GEMINI_BILLING_ENABLED` | `true`, `false` | `false` | Skip rate limiting if billing is enabled on Google Cloud |
...
```json
{
  ...
  "KEEPMIND_GEMINI_API_KEY": "your-api-key-here",
  "KEEPMIND_GEMINI_MODEL": "gemini-2.5-flash-lite",
  "KEEPMIND_GEMINI_BILLING_ENABLED": "true"
}
```
...
Either:
- Set `KEEPMIND_GEMINI_API_KEY` in `~/.keepmind/settings.json`, or
...
keepmind enforces these limits automatically with built-in delays between requests. Processing may be slower but stays within limits.
...
<Tip>
**Recommended**: Enable billing on your Google Cloud project to unlock much higher rate limits. You won't be charged unless you exceed the generous free quota. This allows keepmind to process observations instantly instead of waiting between requests.
</Tip>

If you hit rate limits:
- keepmind automatically falls back to Claude SDK
