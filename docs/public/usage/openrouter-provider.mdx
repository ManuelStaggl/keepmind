...
keepmind supports [OpenRouter](https://openrouter.ai) as an alternative provider for observation extraction. OpenRouter provides a unified API to access 100+ models from different providers including Google, Meta, Mistral, DeepSeek, and many others—often with generous free tiers.

<Warning>
**Unmaintained since 3.4.0.** `claude` is the provider keepmind is developed and
measured against. This path still works, but the 3.4.0 observer rework was not
ported to it: it still re-sends its whole conversation history on every turn,
compresses every tool use instead of gating cheap ones out first, and takes file
lists from the model rather than from the hook payload. Expect materially more
requests and more input tokens per session than on the Claude path. See
[Configuration → Alternate providers](../configuration) for the capability
table.

Secret redaction is **not** weaker here: it runs in the shared prompt builder
that every provider uses.
</Warning>

<Tip>
**Free Models Available**: OpenRouter offers several completely free models. Note that "free" caps requests rather than tokens, and this path issues more of them than the Claude path does.
</Tip>
...
<Note>
**Default Model**: keepmind uses `xiaomi/mimo-v2-flash:free` by default—a 309B parameter mixture-of-experts model that ranks #1 on SWE-bench Verified and excels at coding and reasoning tasks.
</Note>
...
|---------|--------|---------|-------------|
| `KEEPMIND_PROVIDER` | `claude`, `gemini`, `openrouter` | `claude` | AI provider for observation extraction |
| `KEEPMIND_OPENROUTER_API_KEY` | string | — | Your OpenRouter API key |
| `KEEPMIND_OPENROUTER_MODEL` | string | `xiaomi/mimo-v2-flash:free` | Model identifier (see list above) |
| `KEEPMIND_OPENROUTER_MAX_CONTEXT_MESSAGES` | number | `20` | Max messages in conversation history |
| `KEEPMIND_OPENROUTER_MAX_TOKENS` | number | `100000` | Token budget safety limit |
| `KEEPMIND_OPENROUTER_SITE_URL` | string | — | Optional: URL for analytics attribution |
| `KEEPMIND_OPENROUTER_APP_NAME` | string | `keepmind` | Optional: App name for analytics |
...
```json
{
  ...
  "KEEPMIND_OPENROUTER_API_KEY": "sk-or-v1-your-key-here",
...
|-------|----------------------|----------|
| `anthropic/claude-haiku-4.5` | $1 in / $5 out | Best value for compression |
| `anthropic/claude-sonnet-5` | $2 in / $10 out | Highest quality observations |
...
If OpenRouter errors, keepmind logs the failure and re-throws so the message stays pending for later retry. There is no Claude SDK fallback — earlier docs claimed automatic Claude fallback, but the wiring was never actually engaged in production (#2087). To switch providers, change `KEEPMIND_PROVIDER` in settings.
...
Either:
- Set `KEEPMIND_OPENROUTER_API_KEY` in `~/.keepmind/settings.json`, or
...
- Reduce `KEEPMIND_OPENROUTER_MAX_CONTEXT_MESSAGES`
- Reduce `KEEPMIND_OPENROUTER_MAX_TOKENS`
