...
keepmind provides persistent memory across sessions through **4 MCP tools** that follow a token-efficient **3-layer workflow pattern**.

## Overview

Instead of fetching all historical data upfront (expensive), keepmind uses a progressive disclosure approach:
...
**Database:** SQLite FTS5 full-text search on `~/.keepmind/keepmind.db`
