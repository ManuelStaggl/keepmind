...
description: "Add persistent memory to Gemini CLI with keepmind"
---
...
Gemini CLI starts every session from scratch. keepmind changes that by capturing observations, decisions, and patterns — then injecting relevant context into each new session.

<Info>
**How it works:** keepmind installs lifecycle hooks into Gemini CLI that capture tool usage, agent responses, and session events. A local worker service extracts semantic observations and injects relevant history at session start.
</Info>
...
keepmind needs an AI provider to extract observations from your sessions. Choose one:
...
<Tabs>
  <Tab title="Gemini API (Free)">
    ...
    cat > ~/.keepmind/settings.json << 'EOF'
    {
      ...
      "KEEPMIND_GEMINI_API_KEY": "YOUR_API_KEY"
    }
    EOF
    ```
...
# Check hooks are installed — look for keepmind entries
cat ~/.gemini/settings.json | grep keepmind
```
...
Launch Gemini CLI normally. keepmind works in the background:

```bash
gemini
```

On session start, you'll see keepmind context injected with your recent observations and project history.

## What gets captured

keepmind registers 8 of Gemini CLI's 11 lifecycle hooks:
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with keepmind commands.
...
3. Check your AI provider is configured in `~/.keepmind/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your keepmind version may need updating:
