...
keepmind uses an **MCP-based search architecture** that provides intelligent memory retrieval through 3 streamlined tools following a 3-layer workflow pattern.
...
**Key Components**:
1. **MCP Tools** (3 tools) - `search`, `timeline`, `get_observations`
...
6. **In-process vector search** - `sqlite-vec` inside the same database, no separate service
...
Claude has access to 3 MCP tools. When searching memory, Claude follows the 3-layer workflow:
...
The 3-layer workflow used to be a fourth tool of its own (`important_workflow`).
It is now carried in the description of `search`, where the sequence starts — as
a separate entry it spent 396 characters of every session's tool listing to say
what 298 characters say here.
...
The 3-layer workflow embodies **progressive disclosure** - a core principle of keepmind's architecture.
...
### Then: Streamlined MCP Implementation
...
### Now: The Listing Itself Is the Cost

The count kept growing again — smart tools, corpus tools, and a second set bound
to the server runtime — until the listing was **13,003 characters** paid on every
turn, whether or not anything called them.

**Approach:** the listing is filtered twice before it is sent.

- **By runtime.** Only one backend is ever selected, so the tools bound to the
  other one can do nothing but explain that they are unavailable.
- **By group.** `KEEPMIND_MCP_SMART_TOOLS` and `KEEPMIND_MCP_CORPUS_TOOLS` are
  both `false` by default; see [Configuration](/configuration).
- **`important_workflow` folded into `search`**, where the sequence starts.

**Result:** a default worker install lists 5 tools in **3,954 characters** —
about 2,510 tokens per session less than before.

Dispatch is deliberately *not* filtered. A client holding a stale listing still
reaches the handler, which names the setting to change, instead of getting a
bare `Unknown tool`.
...
**Now:** Progressive disclosure is enforced by tool design itself
...
- Can't search without reading the sequence — it is the description of `search`
...
2. Check database: `ls ~/.keepmind/keepmind.db`
