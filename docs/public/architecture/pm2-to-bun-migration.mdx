...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing keepmind for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
keepmind version 7.1.0 introduces two major architectural migrations:
...
- **Logs**: `~/.pm2/logs/keepmind-worker-*.log`
- **PID File**: `~/.pm2/pids/keepmind-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop keepmind-worker   # Stop worker
pm2 restart keepmind-worker # Restart worker
pm2 delete keepmind-worker  # Remove from PM2
pm2 logs keepmind-worker    # View logs
```
...
- **Logs**: `~/.keepmind/logs/worker-YYYY-MM-DD.log`
- **PID File**: `~/.keepmind/.worker.pid`
- **Port File**: `~/.keepmind/.worker.port` (new)
...
- Database file: `~/.keepmind/keepmind.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'keepmind-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.keepmind/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.keepmind/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete keepmind-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.keepmind/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete keepmind-worker` (old worker terminated)
5. **Marker creation**: `~/.keepmind/.pm2-migrated` with timestamp
...
| `pm2 stop keepmind-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart keepmind-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete keepmind-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs keepmind-worker` | `npm run worker:logs` | View logs |
| `pm2 describe keepmind-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/keepmind-worker-out.log
     ~/.pm2/logs/keepmind-worker-error.log

New: ~/.keepmind/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/keepmind-worker.pid

New: ~/.keepmind/.worker.pid
```
...
New: ~/.keepmind/.worker.pid
     ~/.keepmind/.worker.port
     ~/.keepmind/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.keepmind/keepmind.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ keepmind-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove keepmind logs
rm -f ~/.pm2/logs/keepmind-worker-*.log
rm -f ~/.pm2/pids/keepmind-worker.pid
```
...
```
~/.keepmind/
├── keepmind.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── keepmind-worker-out.log
│   └── keepmind-worker-error.log
├── pids/
│   └── keepmind-worker.pid
└── pm2.log
```
...
```
~/.keepmind/
├── keepmind.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `keepmind-worker`
...
**Manual Resolution**:
```bash
rm ~/.keepmind/.worker.pid
rm ~/.keepmind/.worker.port
...
| `Invalid port X` | Port validation failed | Update `KEEPMIND_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name keepmind-worker
...
# 5. Verify migration
pm2 list  # Should NOT show keepmind-worker
cat ~/.keepmind/.pm2-migrated  # Should exist
