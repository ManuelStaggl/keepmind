...
- **Port**: Per-user default `37700 + (uid % 100)` (override with `CLAUDE_MEM_WORKER_PORT`). The active port is stored in `~/.keepmind/settings.json` and reported by `GET /api/health`.
...
- **Override**: Set `CLAUDE_MEM_WORKER_PORT` (env or `~/.keepmind/settings.json`)
