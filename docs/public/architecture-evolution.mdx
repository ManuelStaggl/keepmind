...
description: "How keepmind evolved from v3 to v5+"
---
...
This is the story of how keepmind evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('keepmind-theme') || 'system';
  });

  useEffect(() => {
    localStorage.setItem('keepmind-theme', theme);
  }, [theme]);
...
**Impact**: Cross-platform compatibility restored, Windows users can now use keepmind without issues.
...
```bash
cp ~/.keepmind/keepmind.db ~/.keepmind/keepmind-v3-backup.db
```
