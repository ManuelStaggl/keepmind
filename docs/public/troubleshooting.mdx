...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `CLAUDE_MEM_WORKER_PORT` in `~/.keepmind/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .CLAUDE_MEM_WORKER_PORT ~/.keepmind/settings.json)
   lsof -i :$PORT
...
jq -r .CLAUDE_MEM_WORKER_PORT ~/.keepmind/settings.json
