...
description: "Common issues and solutions for keepmind"
---
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `KEEPMIND_WORKER_PORT` in `~/.keepmind/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .KEEPMIND_WORKER_PORT ~/.keepmind/settings.json)
   lsof -i :$PORT
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('keepmind-settings')
   ```
...
3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('keepmind-settings')
   ```
...
3. Check database for corruption:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "PRAGMA integrity_check;"
   ```
...
3. Verify database has observations:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "SELECT COUNT(*) FROM observations;"
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
3. Check database size (large database = slow queries):
   ```bash
   ls -lh ~/.keepmind/keepmind.db
   ```

4. Optimize database:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "VACUUM;"
   ```
...
2. Backup database:
   ```bash
   cp ~/.keepmind/keepmind.db ~/.keepmind/keepmind.db.backup
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
2. Verify FTS5 tables populated:
   ```bash
   sqlite3 ~/.keepmind/keepmind.db "SELECT COUNT(*) FROM observations_fts;"
   ```
...
**Symptoms**: `/plugin install keepmind` fails.
...
### Plugin Runtime Dependencies Missing

**Symptoms**: Semantic search silently falls back to keyword/FTS results, the
worker logs `Cannot find module 'sqlite-vec'`, or `npx keepmind doctor` reports
`Plugin deps  not resolvable`.

This is about the *installed plugin*, not this repository — the section above is
for development checkouts.

**Where they live**: `~/.claude/plugins/data/keepmind-keepmind/node_modules`.
Note this is **not** the marketplace directory: that one holds code only, and the
absence of `node_modules` there is expected, not a fault.

**Why they go missing**: Claude Code restores the plugin directory from git when
the plugin updates, which deletes gitignored paths. Dependencies moved to the
plugin data directory precisely because it survives that; an installation that
predates the move still keeps its tree beside the bundle and keeps working until
its next install.

**Solutions**:

1. Check what is actually resolving:
   ```bash
   npx keepmind doctor
   ```
   A healthy install reports the data directory. `at legacy location …` means it
   works but has not been migrated yet — reinstall to move it.
...
3. The worker also self-heals: if nothing resolves, it reinstalls before its next
   spawn. A repair that fails is latched for 10 minutes so a broken network does
   not trigger an install on every hook — run `npx keepmind doctor` for the
   recorded reason rather than waiting it out.
...
jq -r .KEEPMIND_WORKER_PORT ~/.keepmind/settings.json
...
**Solution**: Check `~/.keepmind/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/ManuelStaggl/keepmind/issues)
...
- [Configuration](configuration) - Customize keepmind
