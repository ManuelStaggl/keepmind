...
description: "How keepmind intercepts file reads to save tokens using observation history"
---
...
Prior observations for this file (the Read result below is complete). Detail: get_observations([IDs]) · structure: smart_outline("path").
...
The header is one line rather than four on purpose: this block is injected on
nearly every tracked Read, so 60–80 tokens of fixed preamble is thousands of
tokens over a session.

<Note>
  The `structure: smart_outline(...)` half of that line appears **only** when
  `KEEPMIND_MCP_SMART_TOOLS` is `true`. With the smart tools off — the default —
  the hint is omitted rather than left pointing at a tool the model cannot see:
  a suggestion that fails at the follow-up call is worse than no suggestion.
</Note>
...
| **smart_outline / smart_unfold** | ~1-2k | Need current code structure or a specific function (requires `KEEPMIND_MCP_SMART_TOOLS=true`) |
...
- **Observations from months ago** -- file may have changed, consider a full read (or `smart_outline`, if the smart tools are enabled)
...
Projects matching patterns in `KEEPMIND_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.keepmind/settings.json`:

```json
{
  "KEEPMIND_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
2. Find the keepmind hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of keepmind's layered context strategy:
