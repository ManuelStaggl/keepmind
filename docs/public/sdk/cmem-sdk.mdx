...
description: "Complete API reference for keepmind/sdk — the in-process capture → compress → semantic-search pipeline. No HTTP worker, no Redis, no daemon."
---

`keepmind/sdk` (the CMEM-SDK) is the in-process programmatic interface to
...
runnable example, see [Using keepmind in your app](/sdk).

## What it is and when to use it

The SDK is a thin glue layer over the same modules the keepmind server
...
Reach for the SDK when you want to embed keepmind's memory primitives
...
|----------------------|-----------|----------------------------------------------------------------------------------------------|
| **Postgres**         | Yes       | Reached via `KEEPMIND_SERVER_DATABASE_URL`, `options.databaseUrl`, or a supplied `pg.Pool`. System of record. |
...
keepmind without semantic search is considered broken.
</Warning>

## Installation

The SDK ships inside the same `keepmind` package the plugin uses:

```bash
npm install keepmind
```

Import from the `keepmind/sdk` subpath export:
...
1. **A reachable Postgres**, via `KEEPMIND_SERVER_DATABASE_URL` (or
...
// Reads KEEPMIND_SERVER_DATABASE_URL and (for generate) ANTHROPIC_API_KEY.
...
|----------|----------|-------|
| `KEEPMIND_SERVER_DATABASE_URL` | Postgres connection | Fallback when neither `options.databaseUrl` nor `options.pool` is given. |
| `KEEPMIND_DATA_DIR` | Data directory | Defaults to `$HOME/.keepmind`. Holds `sdk-tenant.json` (the persisted bootstrap tenant) and Chroma data. |
| `KEEPMIND_SERVER_PROVIDER` | Provider selection | `'claude'`, `'gemini'`, or `'openrouter'`. Empty/unset falls through to whichever API key is present. |
| `KEEPMIND_SERVER_MODEL` | Model override | Optional model id for the env-resolved provider. |
| `ANTHROPIC_API_KEY` | Claude API key | Also accepts `KEEPMIND_ANTHROPIC_API_KEY`. Default provider when set. |
| `GEMINI_API_KEY` | Gemini API key | Also accepts `KEEPMIND_GEMINI_API_KEY`. |
| `OPENROUTER_API_KEY` | OpenRouter API key | Also accepts `KEEPMIND_OPENROUTER_API_KEY`. |
| `KEEPMIND_OPENROUTER_BASE_URL` | OpenRouter base URL | Also accepts `OPENROUTER_BASE_URL`. OpenRouter only. |
...
`$KEEPMIND_DATA_DIR/sdk-tenant.json`, reusing them on subsequent runs —
fine for local development and single-tenant CLIs, not for multi-tenant
servers.

## See also

- [Using keepmind in your app](/sdk) — narrative walkthrough and a runnable example.
