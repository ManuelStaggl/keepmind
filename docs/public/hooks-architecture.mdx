# How keepmind Uses Hooks: A Lifecycle-Driven Architecture
...
keepmind is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** keepmind doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
keepmind had several architectural constraints:
...
keepmind uses lifecycle hook scripts across 5 lifecycle events. Runtime setup is handled out-of-band by `npx keepmind install` (and `npx keepmind repair`); the Setup hook only runs a sub-100ms `version-check.js` to flag stale installs. SessionStart runs 2 hooks in sequence: worker-service start, then context-hook.

### Setup Hook: Version Check

**Purpose:** Detect stale installs caused by external plugin upgrades and prompt the user to repair.

**Note:** Runtime installation (Bun, uv, `bun install` in the plugin cache) is performed by `npx keepmind install` and `npx keepmind repair` — the Setup hook itself never installs anything.
...
3. On mismatch, writes `run: npx keepmind repair` to stderr.
...
**Source:** `scripts/version-check.js`. The matching installer logic lives in `npx keepmind install` / `npx keepmind repair`, which install Bun + uv globally, run `bun install` in the plugin cache, and write the `.install-version` marker — all behind a visible clack spinner.
...
- ✅ Configurable observation count via `KEEPMIND_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [keepmind] recent context
...
**Technology:** Express.js REST API on the worker's per-user port (default `37700 + (uid % 100)`, override via `KEEPMIND_WORKER_PORT`)
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.keepmind/keepmind.db "SELECT * FROM observation_queue"`
...
- All heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens in `npx keepmind install` / `npx keepmind repair`, which run with a visible clack spinner outside the session lifecycle.
- On marker mismatch the hook prints a one-line `run: npx keepmind repair` hint to stderr and exits 0; the user opts into the slow path explicitly.
...
**Privacy guarantees:**
- All data stored locally in `~/.keepmind/keepmind.db`
...
*The hook-driven architecture enables keepmind to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
