...
| `src/shared/EnvManager.ts` | Stores credentials in `~/.keepmind/.env`, blocks shell-leaked auth vars, and injects only explicit claude-mem credentials |
...
For claude-mem, that value goes in `~/.keepmind/.env`, not your shell, so the background worker uses it consistently across restarts.
...
<Note>
The alias in `model_name` must match `KEEPMIND_MODEL`, or `KEEPMIND_MODEL` must be changed to match your LiteLLM alias. claude-mem does not translate model names.
</Note>
...
`KEEPMIND_MODEL` is passed through to the Claude Agent SDK. In gateway mode, claude-mem allows any non-empty model string because the valid model list is owned by LiteLLM.
...
The model name sent by claude-mem does not match a LiteLLM `model_name`. Make `KEEPMIND_MODEL` and the LiteLLM alias match exactly.
