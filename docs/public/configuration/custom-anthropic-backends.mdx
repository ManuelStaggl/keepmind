...
`CLAUDE_MEM_MODEL` (in `~/.keepmind/settings.json`) is passed straight through to the SDK. The model name **must be one your bridge accepts** — claude-mem does not translate names.
