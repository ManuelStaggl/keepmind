...
|---------|---------|-------------|-----|
| KEEPMIND_MAX_CONCURRENT_AGENTS | 2 | 3 | Better throughput without overload |
| KEEPMIND_SEMANTIC_INJECT | true | true | Relevant context >> recent context |
| KEEPMIND_SEMANTIC_INJECT_LIMIT | 5 | 5 | Sweet spot for token cost vs coverage |
| KEEPMIND_TIER_ROUTING_ENABLED | true | true | ~52% cost savings, no quality loss |
...
If running keepmind on multiple machines, use `keepmind-sync` to keep observations in sync:

```bash
keepmind-sync push <remote-host>    # local -> remote
keepmind-sync pull <remote-host>    # remote -> local
keepmind-sync sync <remote-host>    # bidirectional
keepmind-sync status <remote-host>  # compare counts
```
...
```bash
# Count errors by day
grep '\[ERROR\]' ~/.keepmind/logs/keepmind-*.log | \
  sed 's/\[20[0-9][0-9]-[0-9][0-9]-/\n&/g' | \
  grep -oP '^\[20\d{2}-\d{2}-\d{2}' | sort | uniq -c

# Find circuit-breaker trips
grep 'circuit\|Circuit\|ABANDONED\|abandoned' ~/.keepmind/logs/keepmind-*.log

# Check pending message health
grep 'CLAIMED\|CONFIRMED\|FAILED\|ABANDONED' ~/.keepmind/logs/keepmind-$(date +%Y-%m-%d).log | tail -20
```
