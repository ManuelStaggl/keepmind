...
keepmind includes dual-tag system for content privacy:
...
Review your provider/Chroma configuration in `~/.keepmind/settings.json` and `~/.keepmind/.env` before sending sensitive content. Use `<private>...</private>` tags to keep specific content out of the local store.
