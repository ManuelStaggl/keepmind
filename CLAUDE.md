...
- **Redaction happens on the OUTBOUND path.** `src/sdk/prompts.ts` is the only
  ...
  `src/services/redaction/outbound.ts` first. Redaction downstream of the prompt
  builder does not protect the network: raw tool content — file contents, shell
  output, verbatim user prompts — then reaches the provider unredacted. Do not
  add a prompt-building path that bypasses `prompts.ts`, and do not move
  redaction back downstream.
...
deletes `node_modules`, and `autoUpdate:false` does not prevent that.
`${CLAUDE_PLUGIN_DATA}` survives updates.
...
npm run release:patch -- --title="keepmind <version> — <headline>"   # or :minor / :major
```
...
Three rules the script enforces. Each has been violated in practice, and each
violation was permanent:

- **A release without notes is not a release.** `CHANGELOG.md` is generated from
  GitHub Releases, so a tag without one is a permanent hole in the file. The
  preflight refuses to run on an empty `RELEASE_NOTES.md`.
- **Never `git push --tags`.** The pre-fork claude-mem tags live under
...
- **Tags outside `upstream/` that are unreachable from `main` are a bug.** They
  silently occupy future versions. The preflight fails if any reappear.
...
The dependency-upgrade routine is a COMMAND, not a standing instruction:
`.claude/commands/daily-maintenance.md`. It was prose in this file, which loads
into every session — an open licence to bump majors in the middle of a bugfix.
