...
## ✅ Deploy + Windows-Sync-Fixes + RAM-Blowup-Fix — DURCHGEFÜHRT (2026-07-01)
**Alles gepusht** (origin/main = `1cfde7f`, ahead 0) und **deployed** (Marktplatz + Cache 1.0.0 synchronisiert, Worker neu, pid 55932 @ Port 37777, `status:ok/mcpReady`). Live-DB ist `keepmind.db` (Rename lief beim ersten echten Restart, verlustfrei). Backfill **„complete for all projects"** → Watermarks persistiert → künftige Restarts sofort. Live-Observation-Sync wieder aktiv, RAM **181MB idle**.
- **`fix(build)` (07bcf22-Bereich):** `sync-marketplace.cjs` nutzte `rsync` (auf Windows nicht vorhanden → Sync scheiterte). Ersetzt durch cross-platform **node-Mirror** (gleiche Excludes, Hard-Schutz für `.git`/`node_modules`, erhält Cache-Bookkeeping `.install-version`/`.in_use`). Zweiter Bug: `build-and-sync`-Restart-Substep `cd ~/...` — `~` expandierte nicht in npms Windows-Shell → neues `scripts/restart-marketplace-worker.cjs` (homedir in node). Dry-Run vor echtem Sync verifiziert (löscht nur die 21+3 gewollten Alt-Dateien).
- **`fix(vector)` `1cfde7f` — 7.5GB→1.2GB Backfill-RAM:** Der Startup-Backfill embeddete 100er-Batches × 3 parallele Projekte über die geteilte onnxruntime-Session. transformers.js paddet auf die längste Sequenz → Attention-Scratch O(batch·heads·seqLen²) ≈ 1.8GB/Batch × 3 ≈ 7.5GB (live beobachtet). Fix: `EmbedderService.embed` micro-batcht (16, `CLAUDE_MEM_EMBED_BATCH`) + **serialisiert** alle Inferenz (nur ein Padding-Tensor resident, disposed je Batch); `VectorSync.BACKFILL_CONCURRENCY_LIMIT` 3→1. **Isoliert verifiziert:** Full-Re-Embed gegen DB-Kopie mit leerem Vektorstore → Peak **1190MB** (war ~7500MB), flach (kein Leak). Idle fällt via 5-Min-Embedder-Unload auf ~180MB.
