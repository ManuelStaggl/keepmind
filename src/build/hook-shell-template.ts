 ...
 *   4. $_C/plugins/marketplaces/keepmind/plugin (marketplace install)
 */
...
function candidateBlock(options: ShellTemplateOptions): string {
  ...
  const allGlobs = [...extraCacheRoots, '$_C/plugins/cache/keepmind/keepmind']
    .map((root) => `"${root}"/[0-9]*/`)
    .join(' ');
  lines.push(`ls -dt ${allGlobs} 2>/dev/null;`);
  lines.push(`printf '%s\\n' "$_C/plugins/marketplaces/keepmind/plugin";`);
...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  const cacheRoots = [
    ...
    '$_C/plugins/cache/keepmind/keepmind',
  ].map(shTokenToNode);
  const marketplace = shTokenToNode('$_C/plugins/marketplaces/keepmind/plugin');
