...
function checkMarketplaceMarker(): void {
  try {
    ...
    const marketplaceCandidates = [
      resolve(home, '.claude', 'plugins', 'marketplaces', 'keepmind'),
      resolve(home, '.config', 'claude', 'plugins', 'marketplaces', 'keepmind'),
    ];
    const present = marketplaceCandidates.some(p => p && existsSync(p));
    const cacheCandidates = [
      resolve(home, '.claude', 'plugins', 'cache', 'keepmind', 'keepmind'),
      resolve(home, '.config', 'claude', 'plugins', 'cache', 'keepmind', 'keepmind'),
    ];
    ...
    if (!present && cachePresent) {
      logger.error(
        'SYSTEM',
        'keepmind MCP started but no marketplace directory was found at ~/.claude/plugins/marketplaces/keepmind or the XDG equivalent. The IDE plugin loader needs that directory to fire keepmind hooks (SessionStart, PostToolUse, Stop, etc.). Without it, MCP search will work but no new memories will be captured. To self-heal, run: node ~/.claude/plugins/cache/keepmind/keepmind/*/scripts/smart-install.js (or reinstall the plugin from the marketplace).',
