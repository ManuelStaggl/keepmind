...
import { upsertContextBlock, stripContextBlock, hasContextBlock } from '../shared/context-markers.js';
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.keepmind', '.claude-mem', '.open-next', '.turbo'
  ];
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by keepmind. Edit content outside the tags. -->');
  lines.push('');
...
function writeClaudeMdToFolder(folderPath: string, newContent: string): void {
  ...
  // Writes canonical <keepmind-context> tags and replaces a pre-rename block
  // in place, so an older file is converted instead of gaining a second block.
  const finalContent = upsertContextBlock(existingContent, newContent);
...
export async function generateClaudeMd(dryRun: boolean): Promise<number> {
  ...
  const observationLimit = parseInt(settings.KEEPMIND_CONTEXT_OBSERVATIONS, 10) || 50;
