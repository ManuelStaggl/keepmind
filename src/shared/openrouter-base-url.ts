 ...
 * to turn the optional KEEPMIND_OPENROUTER_BASE_URL setting into a concrete
 * `/chat/completions` endpoint. This makes the OpenRouter client a generic
 * OpenAI-compatible client.
 *
 * Closes #2382 (KEEPMIND_OPENROUTER_BASE_URL), #2590 (custom provider with
 ...
 * KEEPMIND_OPENROUTER_MODEL setting (passed verbatim), a user points the base
 * URL at any OpenAI-compatible endpoint and selects an arbitrary model id.
 *
 * Usage examples (set KEEPMIND_PROVIDER=openrouter, then):
 *
 *   DeepSeek:
 *     KEEPMIND_OPENROUTER_BASE_URL = https://api.deepseek.com
 *     KEEPMIND_OPENROUTER_MODEL    = deepseek-chat
 *     OPENROUTER_API_KEY (in ~/.keepmind/.env) = <deepseek key>
 *
 *   LM Studio (local, no key required):
 *     KEEPMIND_OPENROUTER_BASE_URL = http://localhost:1234/v1
 *     KEEPMIND_OPENROUTER_MODEL    = <any local model id>
 *
 *   Generic OpenAI-compatible endpoint:
 *     KEEPMIND_OPENROUTER_BASE_URL = https://my-gateway.example.com/v1
 *     KEEPMIND_OPENROUTER_MODEL    = <model id>
 */
