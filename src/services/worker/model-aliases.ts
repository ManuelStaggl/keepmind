 ...
 *
 * Lets users write a portable tier alias in KEEPMIND_MODEL (e.g.
...
export function resolveTierAlias(model: string, settings: SettingsDefaults): string {
  ...
  switch (match[1]) {
    case 'fast':
      return settings.KEEPMIND_TIER_FAST_MODEL || 'haiku';
    case 'smart':
      return settings.KEEPMIND_TIER_SMART_MODEL || 'sonnet';
    case 'simple':
      return settings.KEEPMIND_TIER_SIMPLE_MODEL || 'haiku';
    case 'summary':
      ...
      return settings.KEEPMIND_TIER_SUMMARY_MODEL || settings.KEEPMIND_MODEL;
    default:
      return model;
  }
}
