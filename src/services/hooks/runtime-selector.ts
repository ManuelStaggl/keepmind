...
//
// Reads `KEEPMIND_RUNTIME` from `~/.keepmind/settings.json` (via
...
// existing settings.json files and `KEEPMIND_RUNTIME` values keep
// working. Likewise, new settings keys `KEEPMIND_SERVER_{URL,API_KEY,
// PROJECT_ID}` are read first and fall back to the legacy
// `KEEPMIND_SERVER_BETA_*` keys when unset.
...
export function selectRuntime(): SelectedRuntime {
  ...
  const raw = (settings.KEEPMIND_RUNTIME ?? 'worker').trim().toLowerCase();
...
export function buildServerContext(): ServerRuntimeContext | null {
  ...
  const serverBaseUrl = pickFirstNonEmpty(
    settings.KEEPMIND_SERVER_URL,
    settings.KEEPMIND_SERVER_BETA_URL,
  );
  const apiKey = pickFirstNonEmpty(
    settings.KEEPMIND_SERVER_API_KEY,
    settings.KEEPMIND_SERVER_BETA_API_KEY,
  );
  const projectId = pickFirstNonEmpty(
    settings.KEEPMIND_SERVER_PROJECT_ID,
    settings.KEEPMIND_SERVER_BETA_PROJECT_ID,
  );
