...
import { CuratedRoutes } from './worker/http/routes/CuratedRoutes.js';
...
export class WorkerService implements WorkerRef {
  ...
  private registerRoutes(): void {
    ...
    // Lasting entries authored in-session. Plain-storage writes only — see
    // src/services/curated/authoring.ts for why no model is reachable here.
    this.server.registerRoutes(new CuratedRoutes(this.dbManager, 'keepmind'));
...
export function parseWorkerServiceCommand(argv: string[]): ParsedWorkerCommand {
  ...
  if (rawCommand === 'server') {
    // Local-only fork: the cloud server runtime (Postgres + BullMQ) was removed.
    // The only surviving `server` subcommand is `api-key`, which manages local
    // SQLite API keys used by the local v1 routes.
...
function printServerCommandHelp(): never {
  ...
  console.error('Commands: api-key create|list|revoke|migrate-scopes');
  process.exit(1);
}
