...
export async function notifyTelegram(input: TelegramNotifyInput): Promise<void> {
  ...
  if (settings.KEEPMIND_TELEGRAM_ENABLED !== 'true') {
    return;
  }

  const botToken = settings.KEEPMIND_TELEGRAM_BOT_TOKEN;
  const chatId = settings.KEEPMIND_TELEGRAM_CHAT_ID;
  if (!botToken || !chatId) {
    return;
  }

  const triggerTypes = splitCsv(settings.KEEPMIND_TELEGRAM_TRIGGER_TYPES);
  const triggerConcepts = splitCsv(settings.KEEPMIND_TELEGRAM_TRIGGER_CONCEPTS);
